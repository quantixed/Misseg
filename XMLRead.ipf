...
// This function is not required yet - keeping for future use
Function WorkOnImage()
	WAVE/Z imgMat
	WAVE/Z mat_1,mat_2,mat_3,mat_4,mat_5
	MakeBgWave(imgMat,mat_5)
	MeasureIntensities(mat_1,imgMat)
	MeasureIntensities(mat_2,imgMat)
	MeasureIntensities(mat_3,imgMat)
...
Function/WAVE MakeBgWave(ImageMat,bgMat)
	...
	Variable nChannels = dimsize(ImageMat,2) // check this
	Make/O/N=(nPoints,nChannels) bgValueW
	for(i = 0; i < nPoints; i += 1)
		...
		for(j = 0; j < nChannels; j += 1)
			WaveStats/M=1/RMD=[xPosMin,xPoxMax][yPosMin,yPosMax][zPos][j] ImageMat
			bgValueW[i][j] = V_Avg
		endfor
	endfor
	MatrixOp/O bgW = sumcols(bgValueW) / numcols(bgValueW)	// check this!
	return bgW
End

// This function is not required yet - keeping for future use
// offset of 1!!
Function MeasureIntensities(objToMeasure,ImageMat,bgMat)
	Wave objToMeasure,ImageMat,bgMat
	WAVE/Z bgValueW
	if(!WaveExists(bgValueW))
		Variable nPoints = dimsize(bgmat,0)
		Variable nChannels = dimsize(ImageMat,2) // check this
		Make/O/N=(nPoints,nChannels) bgValueW
		bgValueW[i][j] = 

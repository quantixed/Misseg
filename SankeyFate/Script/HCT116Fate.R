...
# load all data
df_data <- read.delim("Data/HCT116_fate.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# fate is [0,3] we need [3,6] for sankey diagram
df_data$Fate <- df_data$Fate + 3

# control data
df_control <- subset(df_data, startsWith(as.character(Experiment),"Control"))
# gsk data
df_gsk <- subset(df_data, startsWith(as.character(Experiment),"GSK"))
